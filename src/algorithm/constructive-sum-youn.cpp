 ...
 */
VertexPtr getBestVertexSumYoun(
 ...
 */
std::vector<VertexPtr> sortVerticesDegreeSumYoun(
    Graph graph,
 ...
 */
std::vector<VertexPtr> sortVerticesSumWeightSumYoun(
    Graph graph,
 ...
 */
void constructiveMEWCRecursiveSumYoun(
    Graph g,
    Clique &clique,
    ...
    std::vector<VertexPtr> &sortedVertices)
{
    ...
    VertexPtr newVertex = getBestVertexSumYoun(P, sortedVertices);
    ...
    constructiveMEWCRecursiveSumYoun(g, clique, new_P, sortedVertices); 
}
 ...
 */
Clique constructiveMEWCSumYoun(Graph g) // O(n^2)
{
    // LaTeX : $R \gets \emptyset$
    Clique clique;
    // LaTeX : $P \gets V$
    std::unordered_set<VertexPtr> P = g.getVertices();
    std::vector<VertexPtr> sortedVertices = sortVerticesSumWeightSumYoun(g, P); // O(nlogn)
    // std::vector<VertexPtr> sortedVertices = sortVerticesSumWeight(g, P); // O(n^2)

    // LaTeX : \Call{ConstructiveRecursiveMEWC}{$R$, $P$}
    constructiveMEWCRecursiveSumYoun(g, clique, P, sortedVertices); // O(n^2)
